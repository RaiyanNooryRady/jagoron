...
function get_the_post_custom_thumbnail($post_id, $size = 'featured-thumbnail', $additional_attributes = [])
{
    ...
    $attributes = array_merge($additional_attributes, $default_attiributes);
...
function jagoron_posted_on()
{
    ...
    $posted_on = sprintf(
        esc_html_x('Posted on %s', 'post date', 'jagoron'),
...
function jagoron_the_excerpt($trim_character_count=0){
...
function jagoron_excerpt_more($more=''){
    if(!is_single()){
        $more=sprintf('<button class="mt-4 btn btn-info"><a class="jagoron-read-more text-white" href="%1$s">%2$s</a></button>',get_permalink(get_the_ID()),__('Read More','jagoron'));
    }
    return $more;
}